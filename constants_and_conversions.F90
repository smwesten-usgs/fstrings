...
!--------------------------------------------------------------------------
  ...
  logical ( kind=c_bool ) function is_numeric( value )
...
elemental function fortran_to_c_string( sText )  result(cCharacterString)
  ...
  subroutine assert( logical_test, text, file_text, lineno )

    logical,               intent(in) :: logical_test
    character (len=*), intent(in)     :: text
    character (len=*), intent(in)     :: file_text
    integer (kind=c_int), intent(in)  :: lineno

    if ( .not. logical_test ) then
      print *, 'msg:  ', text
      print *, 'file: ', trim(file_text)
      print *, 'line: ', lineno
      stop
    endif
  ...
  subroutine die( text, file_text, lineno )
